...
from generateCompleteTree import CRFNFL_SVM
from sklearn.svm import SVC
import numpy as np
import pandas as pd


def kernel_SVM(train, test=None):
...
def main_train(key, noise, divide_num=5):
    ...
    print("KSVM:%.4lf" % (round_acc[0]), "CRF:%.4lf" % (round_acc[1]),
          "RD:%.4lf" % (round_acc[2]), "KSVM:%.4lf" % (round_accstd[0]), "CRF:%.4lf" % (round_accstd[1]),
...
def main_KSVM():
    ...
    noises = [(0.2, 0.2), (0.3, 0.3), (0.4, 0.4), (0.1, 0.3), (0.3, 0.1), (0.1, 0.4)]
    ...
    # temp_all = pd.DataFrame(acc_all, columns=["KSVM", "CRF", "RD"])
    # temp_all.to_excel(r"C:\Users\ASUS\Desktop\毕设实验\record data/kernel_SVM.xlsx", index=False)
    #
    # temp_all1 = pd.DataFrame(acc_all1, columns=["KSVM", "CRF", "RD"])
    # temp_all1.to_excel(r"C:\Users\ASUS\Desktop\毕设实验\record data/kernel_SVM_std.xlsx", index=False)
